      ...
      {(title || subtitle) && (
        ...
        >
          <div className="h-[150px] w-full relative ">
            {title && (
              <div className={`card-title  ${titleClass} flex justify-center`}>{title}</div>
            )}
            ...
            <div className="bg-[#0C9AD6] h-36 w-36 rounded-full absolute  bottom-[-50%] flex   left-1/2 transform -translate-x-1/2  ring-4 ring-white">
              <div className="flex h-36 w-36 justify-center  items-center text-white">
                <div className="text-center">
                  {saveCost&&<div className="text-base font-mono ">{saveCost}</div>}
                  {paymentMethod&&<div className="font-bold text-5xl">{paymentMethod}</div>}
                  {paymentAmount&&<div className="font-medium text-sm">{paymentAmount}</div>}
